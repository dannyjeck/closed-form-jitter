...
% Figure 2 in "Closed Form Jitter Analysis of Neuronal Spike Trains"
...
% analysis with the Closed Form Jitter Method in computing Jitter-Corrected
% Cross Correlograms (JCCGs).


T_list = 1:10:100; %list of singal lengths (In seconds) to be tested
f_list = [5 10 20 40 50 100 200]; %list of firing frequencies (in Hz) to be tested
repeat = 50; %number of signals tested per condition
...
for fidx = 1:length(f_list)
    for tidx = 1:length(T_list);
        for rep = 1:repeat
            ...
            tic
                [excess] = jitter_closed_form(X1,X2,D,taumax); %only one output means only the JCCG is computed
            times_closed_form(fidx,tidx,rep) = toc;

            tic
                [excess2] = jitter_monte_carlo(X1,X2,D,Ntrials,taumax); %only one output means only the JCCG is computed
...
subplot(121);
imagesc(T_list,f_list,times_closed_form_mean,[0 Imax]);colorbar
axis square;
hold all;plot(T_list,line,'k');hold off;
xlabel('Length of signal (T in Seconds)');
...
for k = 1:length(f_list)
    f = f_list(k);
    txt = [num2str(f) ' Hz'];
    h_txt(k) = text(T_list(3),(times_jitter_mc_mean_bf(k,3)./times_closed_form_mean(k,3)),txt,'BackgroundColor','w');
end
xlabel('Spike Train Length [s]');
ylabel('Performance Gain');
title('JCCG Computation');
